...
public class EncryptFile extends Application {
	TextField fileToEncryptField =  new TextField();
	TextField fileToSaveField =  new TextField();
	TextField outputField = new TextField();
	
	public void handleButtonEvent() {
		outputField.clear();
		String inputFile = fileToEncryptField.getText();
		...
		if (!inputFile.isBlank() && !outputFile.isBlank()) {
			...
			if (inputFile.isBlank()) 
				outputField.appendText("File to encrypt not chosen");
			else if (outputFile.isBlank()) 
				outputField.appendText("Path to save the encrypted file not chosen");
		}
	}
	...
	public void encrypt(String inputFile, String outputFile) {
		...
		outputField.appendText("File is encrypted and saved");
	...
	public void start(Stage primaryStage) {
		...
		fileToEncryptField = new TextField();
		fileToEncryptField.setPrefWidth(300);
		fileToEncryptField.setPromptText("File to encrypt");
	
		fileToSaveField.setPrefWidth(300);
		fileToSaveField.setPromptText("Save encrypted file to");
		...
		top.getChildren().addAll(fileToEncryptField, fileToSaveField, btnEncrypt);
